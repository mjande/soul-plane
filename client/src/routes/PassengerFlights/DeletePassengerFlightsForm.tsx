...
export default function DeletePassengerFlightsForm() {
    ...
    async function getPassenger() {
      try {
        const response = await Axios.get(
          `http://${import.meta.env.VITE_BACKEND_HOST}/passengers/${pid}`
        );
  ...
  useEffect(() => {
    async function getFlights() {
      try {
        const response = await Axios.get(
          `http://${import.meta.env.VITE_BACKEND_HOST}/flights/${fid}`
        );
        const data = response.data[0];
        setFlights({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await Axios.delete(
        `http://${import.meta.env.VITE_BACKEND_HOST}/passengerFlights/${fid}/${pid}`,
        {data: formData}
      );
